...
class JournalSectionRestController extends ApiController
{
    ...
    public function getSectionsAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('vipa_api.journal_section.handler')->all($limit, $offset);
    }
     ...
     *   output = "Vipa\SectionBundle\Entity\Section",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postSectionAction(Request $request)
    {
        ...
        try {
            ...
            $newEntity = $this->container->get('vipa_api.journal_section.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putSectionAction(Request $request, $id)
    {
        ...
        try {
            $journalService = $this->container->get('vipa.journal_service');
            if (!($entity = $this->container->get('vipa_api.journal_section.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $entity = $this->container->get('vipa_api.journal_section.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('vipa_api.journal_section.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    public function patchSectionAction(Request $request, $id)
    {
        ...
        try {
            $journalService = $this->container->get('vipa.journal_service');
            $entity = $this->container->get('vipa_api.journal_section.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
            $routeOptions = array(
    ...
    public function deleteSectionAction($id)
    {
        ...
        $entity = $this->getOr404($id);
        $this->container->get('vipa_api.journal_section.handler')->delete($entity);
