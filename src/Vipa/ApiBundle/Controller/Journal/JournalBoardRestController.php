...
class JournalBoardRestController extends ApiController
{
    ...
    public function getBoardsAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('vipa_api.journal_board.handler')->all($limit, $offset);
    }
     ...
     *   output = "Vipa\BoardBundle\Entity\Board",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postBoardAction(Request $request)
    {
        ...
        try {
            ...
            $newEntity = $this->container->get('vipa_api.journal_board.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putBoardAction(Request $request, $id)
    {
        ...
        try {
            $journalService = $this->container->get('vipa.journal_service');
            if (!($entity = $this->container->get('vipa_api.journal_board.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $entity = $this->container->get('vipa_api.journal_board.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('vipa_api.journal_board.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    public function patchBoardAction(Request $request, $id)
    {
        ...
        try {
            $journalService = $this->container->get('vipa.journal_service');
            $entity = $this->container->get('vipa_api.journal_board.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
            $routeOptions = array(
    ...
    public function deleteBoardAction($id)
    {
        ...
        $entity = $this->getOr404($id);
        $this->container->get('vipa_api.journal_board.handler')->delete($entity);
