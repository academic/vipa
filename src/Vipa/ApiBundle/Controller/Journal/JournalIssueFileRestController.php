...
class JournalIssueFileRestController extends ApiController
{
    ...
    public function getFilesAction(ParamFetcherInterface $paramFetcher)
    {
        ...
        $issueFileHandler = $this->container->get('vipa_api.journal_issue_file.handler');
     ...
     *   output = "Vipa\JournalBundle\Entity\IssueFile",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postFileAction(Request $request)
    {
        ...
        try {
            ...
            $newEntity = $this->container->get('vipa_api.journal_issue_file.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putFileAction(Request $request, $id)
    {
        ...
        try {
            $journalService = $this->container->get('vipa.journal_service');
            if (!($entity = $this->container->get('vipa_api.journal_issue_file.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $entity = $this->container->get('vipa_api.journal_issue_file.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('vipa_api.journal_issue_file.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    public function patchFileAction(Request $request, $id)
    {
        ...
        try {
            $journalService = $this->container->get('vipa.journal_service');
            $entity = $this->container->get('vipa_api.journal_issue_file.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
            $routeOptions = array(
    ...
    public function deleteFileAction($id)
    {
        ...
        $this->container->get('vipa_api.journal_issue_file.handler')->delete($entity);
    ...
    {
        if (!($entity = $this->container->get('vipa_api.journal_issue_file.handler')->get($id, $normalize))) {
