...
class JournalIssueRestController extends ApiController
{
    ...
    public function getIssuesAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('vipa_api.journal_issue.handler')->all($limit, $offset);
    }
     ...
     *   output = "Vipa\IssueBundle\Entity\Issue",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postIssueAction(Request $request)
    {
        ...
        try {
            ...
            $newEntity = $this->container->get('vipa_api.journal_issue.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putIssueAction(Request $request, $id)
    {
        ...
        try {
            $journalService = $this->container->get('vipa.journal_service');
            if (!($entity = $this->container->get('vipa_api.journal_issue.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $entity = $this->container->get('vipa_api.journal_issue.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('vipa_api.journal_issue.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    public function patchIssueAction(Request $request, $id)
    {
        ...
        try {
            $journalService = $this->container->get('vipa.journal_service');
            $entity = $this->container->get('vipa_api.journal_issue.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
            $routeOptions = array(
    ...
    public function getAddArticleAction(Request $request, $issueId, $articleId, $sectionId)
    {
        ...
        $section = $em->getRepository('VipaJournalBundle:Section')->find($sectionId);
    ...
    public function deleteIssueAction($id)
    {
        ...
        $this->container->get('vipa_api.journal_issue.handler')->delete($entity);
    ...
    {
        if (!($entity = $this->container->get('vipa_api.journal_issue.handler')->get($id, $normalize))) {
