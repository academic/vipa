...
class AnnouncementRestController extends FOSRestController
{
    ...
    public function getAnnouncementsAction(ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('vipa_api.announcement.handler')->all($limit, $offset);
    }
     ...
     *   output = "Vipa\AnnouncementBundle\Entity\Announcement",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postAnnouncementAction(Request $request)
    {
        ...
        try {
            $newEntity = $this->container->get('vipa_api.announcement.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putAnnouncementAction(Request $request, $id)
    {
        ...
        try {
            if (!($entity = $this->container->get('vipa_api.announcement.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $entity = $this->container->get('vipa_api.announcement.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('vipa_api.announcement.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    {
        try {
            $entity = $this->container->get('vipa_api.announcement.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
    ...
    public function deleteAnnouncementAction($id)
    {
        ...
        $this->container->get('vipa_api.announcement.handler')->delete($entity);
