...
class InstitutionRestController extends FOSRestController
{
    ...
    public function getInstitutionsAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('vipa_api.institution.handler')->all($limit, $offset);
    }
     ...
     *   output = "Vipa\InstitutionBundle\Entity\Institution",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postInstitutionAction(Request $request)
    {
        ...
        try {
            $newEntity = $this->container->get('vipa_api.institution.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putInstitutionAction(Request $request, $id)
    {
        ...
        try {
            if (!($entity = $this->container->get('vipa_api.institution.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $entity = $this->container->get('vipa_api.institution.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('vipa_api.institution.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    {
        try {
            $entity = $this->container->get('vipa_api.institution.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
    ...
    public function deleteInstitutionAction($id)
    {
        ...
        $this->container->get('vipa_api.institution.handler')->delete($entity);
