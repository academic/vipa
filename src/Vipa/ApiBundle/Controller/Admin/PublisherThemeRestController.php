...
class PublisherThemeRestController extends FOSRestController
{
    ...
    public function getPublisherthemesAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('vipa_api.publisher_theme.handler')->all($limit, $offset);
    }
     ...
     *   output = "Vipa\PublisherThemeBundle\Entity\PublisherTheme",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postPublisherthemeAction(Request $request)
    {
        ...
        try {
            $newEntity = $this->container->get('vipa_api.publisher_theme.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putPublisherthemeAction(Request $request, $id)
    {
        ...
        try {
            if (!($entity = $this->container->get('vipa_api.publisher_theme.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $entity = $this->container->get('vipa_api.publisher_theme.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('vipa_api.publisher_theme.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    {
        try {
            $entity = $this->container->get('vipa_api.publisher_theme.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
    ...
    public function deletePublisherthemeAction($id)
    {
        ...
        $this->container->get('vipa_api.publisher_theme.handler')->delete($entity);
