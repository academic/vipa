...
class IndexRestController extends FOSRestController
{
    ...
    public function getIndexesAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('vipa_api.index.handler')->all($limit, $offset);
    }
     ...
     *   output = "Vipa\IndexBundle\Entity\Index",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postIndexesAction(Request $request)
    {
        ...
        try {
            $newEntity = $this->container->get('vipa_api.index.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putIndexesAction(Request $request, $id)
    {
        ...
        try {
            if (!($entity = $this->container->get('vipa_api.index.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $entity = $this->container->get('vipa_api.index.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('vipa_api.index.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    {
        try {
            $entity = $this->container->get('vipa_api.index.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
    ...
    public function deleteIndexesAction($id)
    {
        ...
        $this->container->get('vipa_api.index.handler')->delete($entity);
