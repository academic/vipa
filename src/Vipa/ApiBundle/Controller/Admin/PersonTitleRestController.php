...
class PersonTitleRestController extends FOSRestController
{
    ...
    public function getPersontitlesAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('vipa_api.person_title.handler')->all($limit, $offset);
    }
     ...
     *   output = "Vipa\PersonTitleBundle\Entity\PersonTitle",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postPersontitleAction(Request $request)
    {
        ...
        try {
            $newEntity = $this->container->get('vipa_api.person_title.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putPersontitleAction(Request $request, $id)
    {
        ...
        try {
            if (!($entity = $this->container->get('vipa_api.person_title.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $entity = $this->container->get('vipa_api.person_title.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('vipa_api.person_title.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    {
        try {
            $entity = $this->container->get('vipa_api.person_title.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
    ...
    public function deletePersontitleAction($id)
    {
        ...
        $this->container->get('vipa_api.person_title.handler')->delete($entity);
