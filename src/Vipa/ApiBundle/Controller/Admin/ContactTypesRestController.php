...
class ContactTypesRestController extends FOSRestController
{
    ...
    public function getContacttypesAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('vipa_api.contact_type.handler')->all($limit, $offset);
    }
     ...
     *   output = "Vipa\JournalBundle\Entity\ContactType",
     *   statusCodes = {
     *     200 = "Returned when successful",
     ...
     *   input = "Vipa\JournalBundle\Form\Type\ContactTypesType",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postContacttypeAction(Request $request)
    {
        ...
        try {
            $newContactType = $this->container->get('vipa_api.contact_type.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
    ...
    public function putContacttypeAction(Request $request, $id)
    {
        ...
        try {
            if (!($contactType = $this->container->get('vipa_api.contact_type.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $contactType = $this->container->get('vipa_api.contact_type.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $contactType = $this->container->get('vipa_api.contact_type.handler')->put(
                    $contactType,
                    $request->request->all()
                );
            }
    ...
    {
        try {
            $entity = $this->container->get('vipa_api.contact_type.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
    ...
    public function deleteContacttypeAction($id)
    {
        ...
        $this->container->get('vipa_api.contact_type.handler')->delete($entity);
