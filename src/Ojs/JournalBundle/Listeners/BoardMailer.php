...
class BoardMailer extends AbstractJournalItemMailer
{
    ...
    {
        $getMailEvent = $this->ojsMailer->getEventByName(BoardEvents::POST_CREATE);
        /** @var User $user */
    ...
    {
        $getMailEvent = $this->ojsMailer->getEventByName(BoardEvents::POST_UPDATE);
        /** @var User $user */
        foreach ($this->ojsMailer->getJournalRelatedUsers() as $user) {
            $transformParams = [
                'board'     => (string)$itemEvent->getItem(),
    ...
    {
        $getMailEvent = $this->ojsMailer->getEventByName(BoardEvents::PRE_DELETE);
        /** @var User $user */
        foreach ($this->ojsMailer->getJournalRelatedUsers() as $user) {
            $transformParams = [
                'board'     => (string)$itemEvent->getItem(),
