...
class UserRestController extends FOSRestController
{
    ...
    public function getUsersAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('ojs_api.user.handler')->all($limit, $offset);
    }
    ...
    public function postUserAction(Request $request)
    {
        ...
        try {
            $newEntity = $this->container->get('ojs_api.user.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putUserAction(Request $request, $id)
    {
        ...
        try {
            if (!($entity = $this->container->get('ojs_api.user.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $entity = $this->container->get('ojs_api.user.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('ojs_api.user.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    {
        try {
            $entity = $this->container->get('ojs_api.user.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
