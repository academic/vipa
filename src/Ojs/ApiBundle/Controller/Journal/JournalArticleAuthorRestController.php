...
class JournalArticleAuthorRestController extends ApiController
{
    ...
    public function getAuthorsAction(ParamFetcherInterface $paramFetcher)
    {
        ...
        $articleAuthorHandler = $this->container->get('ojs_api.journal_article_author.handler');
     ...
     *   output = "Ojs\JournalBundle\Entity\ArticleAuthor",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postAuthorAction(Request $request)
    {
        ...
        try {
            ...
            $newEntity = $this->container->get('ojs_api.journal_article_author.handler')->post(
                $request->request->all()
            );
            $routeOptions = array(
                'id' => $newEntity->getId(),
    ...
    public function putAuthorAction(Request $request, $id)
    {
        ...
        try {
            ...
            if (!($entity = $this->container->get('ojs_api.journal_article_author.handler')->get($id))) {
                ...
                $entity = $this->container->get('ojs_api.journal_article_author.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $entity = $this->container->get('ojs_api.journal_article_author.handler')->put(
                    $entity,
                    $request->request->all()
                );
            }
    ...
    public function patchAuthorAction(Request $request, $id)
    {
        ...
        try {
            $journalService = $this->container->get('ojs.journal_service');
            $entity = $this->container->get('ojs_api.journal_article_author.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
            $routeOptions = array(
