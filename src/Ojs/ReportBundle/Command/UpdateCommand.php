...
namespace Ojs\ReportBundle\Command;

use Doctrine\ODM\MongoDB\DocumentManager;
use Doctrine\ORM\EntityManager;
use JMS\Serializer\Serializer;
use Ojs\ReportBundle\Document\ObjectCommonStat;
...
use Ojs\ReportBundle\Updater;
use Ojs\ReportBundle\Updater\UpdaterInterface;
...
/**
 * Class UpdateCommand
 * @package Ojs\ReportBundle\Command
 */
class UpdateCommand extends ContainerAwareCommand
{
    ...
    private function updateViewData(OutputInterface $output)
    {
        try {
            ...
            $allViews = $this->dm->getRepository('OjsReportBundle:ObjectViews')->findAll();
            $counts = [];
            ...
            foreach ($counts as $key => $object) {
                ...
                //check
                $totalView = $this->dm->getRepository("OjsReportBundle:ObjectView")
    ...
    private function updateDownloadData(OutputInterface $output)
    {
        try {
            ...
            $allDownloads = $this->dm->getRepository("OjsReportBundle:ObjectDownloads")->findAll();
            $counts = [];
            ...
            foreach ($counts as $key => $object) {
                ...
                $totalDownload = $this->dm
                    ->getRepository("OjsReportBundle:ObjectDownload")
    ...
    private function updateCommonData()
    {
        ...
        foreach ($updates as $update) {
            $updater = 'Ojs\\ReportBundle\\Updater\\'.$update.'Updater';
            ...
            foreach ($statObject->count() as $objectId => $stat) {
                ...
                $object->setDate(new \DateTime())
                    ...
                    ->setObject($objectId)
                    ->setStatType(constant("Ojs\\ReportBundle\\Document\\ObjectCommonStat::".$update));
